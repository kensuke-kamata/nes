...
pub fn opcodes() -> &'static Opcodes {
    static mut OPCODES: Option<Opcodes> = None;
    static ONCE: Once = Once::new();

    unsafe {
        ONCE.call_once(|| {
            OPCODES = Some(Opcodes::new());
        });
        OPCODES.as_ref().expect("OPCODES is None")
    }
}
...
mod tests {
    ...
    fn test_opcodes() {
        ...
        assert_eq!(opcodes.map[&0x00].mnemonic, "BRK");
        assert_eq!(opcodes.map[&0xaa].mnemonic, "TAX");
        assert_eq!(opcodes.map[&0xe8].mnemonic, "INX");
        assert_eq!(opcodes.map[&0xa9].mnemonic, "LDA");
        assert_eq!(opcodes.map[&0x85].mnemonic, "STA");
    }

    #[test]
    fn test_opcodes_singleton() {
        let opcodes_instance_1 = opcodes();
        let opcodes_instance_2 = opcodes();
        assert_eq!(
            opcodes_instance_1 as *const _,
            opcodes_instance_2 as *const _
        );
    }
}
